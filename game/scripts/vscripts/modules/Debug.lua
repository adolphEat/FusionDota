...
DebugCallbacks = {
    ...
    ["-shop"] = {
        ...
        func = function(____, hero, ...)
            local args = {...}
            local action = args[1] or "show"
            local ____opt_154 = GameRules.GameModeManager
            if not (____opt_154 and ____opt_154:isAutoChessMode()) then
                Say(hero, "This command only works in autochess mode", true)
                return
            end
            repeat
                local ____switch176 = action
                local ____cond176 = ____switch176 == "refresh"
                if ____cond176 then
                    Say(hero, "商店已刷新", true)
                    break
                end
                ____cond176 = ____cond176 or ____switch176 == "show"
                do
    ...
    ["-chess_info"] = {
        ...
        func = function(____, hero, ...)
            local args = {...}
            local pieceId = args[1]
            local ____opt_156 = GameRules.GameModeManager
            if not (____opt_156 and ____opt_156:isAutoChessMode()) then
                Say(hero, "This command only works in autochess mode", true)
                return
            end
            if pieceId then
...
function Debug.prototype.executeDebugStatus(self)
    do
        ...
        local ____try, ____hasReturned = pcall(function()
            ...
            print("[Debug] IsInToolsMode: " .. tostring(IsInToolsMode()))
            print("[Debug] PlayerCount: " .. tostring(PlayerResource:GetPlayerCount()))
            local ____print_159 = print
            local ____hero_158
            if hero then
                ____hero_158 = hero:GetUnitName()
            else
                ____hero_158 = "null"
            end
            ____print_159("[Debug] Hero: " .. ____hero_158)
...
function Debug.prototype.OnPlayerChat(self, keys)
    do
        ...
        local ____try, ____hasReturned, ____returnValue = pcall(function()
            ...
            local hero = HeroList:GetHero(0)
            local ____print_161 = print
            local ____hero_160
            if hero then
                ____hero_160 = hero:GetUnitName()
            else
                ____hero_160 = "null"
            end
            ____print_161("[Debug] Hero found: " .. ____hero_160)
            if cmd == "-test" then
