-- 斗士2羁绊技能 - 全局buff技能
-- 所有友军增加250点最大生命值，斗士单位增加双倍（500点最大生命值）
-- 斗士单位：树精卫士(treant_protector1)、敌法师(anti_mage1)、食人魔法师(ogre_magi1)、斧王(axe1)

-- 全局表：记录每个单位的原始最大生命值（以单位实体索引为键）
Warrior2OriginalHealth = Warrior2OriginalHealth or {}
-- 全局表：记录哪些单位已经被处理过（计数为1的单位不再修改生命值）
Warrior2ProcessedUnits = Warrior2ProcessedUnits or {}
-- 全局表：记录羁绊监听器（用于管理事件监听）
Warrior2Listeners = Warrior2Listeners or {
    list = {},  -- 监听器列表
    event_registered = false,  -- 事件是否已注册
    death_event_registered = false  -- 死亡事件是否已注册
}

LinkLuaModifier("modifier_warrior_2", "heroes/universal/warrior_2", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_warrior_2_effect", "heroes/universal/warrior_2", LUA_MODIFIER_MOTION_NONE)

if warrior_2 == nil then
    warrior_2 = class({})
end

function warrior_2:GetIntrinsicModifierName()
    return "modifier_warrior_2"
end

function warrior_2:GetAbilityTextureName()
    return "beastmaster_primal_roar"
end

-- 全局斗士羁绊效果修饰符
if modifier_warrior_2 == nil then
    modifier_warrior_2 = class({})
end

function modifier_warrior_2:IsHidden()
    return false
end

function modifier_warrior_2:IsPurgable()
    return false
end

function modifier_warrior_2:IsDebuff()
    return false
end

function modifier_warrior_2:IsPassive()
    return true
end

function modifier_warrior_2:GetTexture()
    return "beastmaster_primal_roar"
end

-- 斗士单位列表
function modifier_warrior_2:IsWarriorUnit(unit)
...
function modifier_warrior_2:OnCreated()
    if IsServer() then
        -- 只对玩家团队生效，敌人团队的羁绊不生效
        ...
        -- 注册单位创建事件监听（只在第一次注册）
        if not Warrior2Listeners.event_registered then
            Warrior2Listeners.event_registered = true
            ListenToGameEvent("npc_spawned", function(event)
                modifier_warrior_2:OnNPCSpawned(event)
            end, nil)
        end
        
        -- 注册单位死亡事件监听（只在第一次注册）
        if not Warrior2Listeners.death_event_registered then
            Warrior2Listeners.death_event_registered = true
            ListenToGameEvent("entity_killed", function(keys)
                local killed_unit = EntIndexToHScript(keys.entindex_killed)
                if killed_unit and not killed_unit:IsNull() then
                    modifier_warrior_2:OnUnitDeath(killed_unit)
                end
            end, nil)
        end
        
        -- 将当前 modifier 添加到监听器列表
        table.insert(Warrior2Listeners.list, self)
...
-- 单位死亡事件处理函数（静态方法）
function modifier_warrior_2:OnUnitDeath(dead_unit)
    if not IsServer() then return end
    if not dead_unit or dead_unit:IsNull() then return end
    
    -- 检查是否有活动的羁绊监听器
    if #Warrior2Listeners.list == 0 then return end
    
    -- 检查死亡单位是否是绑定单位（拥有modifier_warrior_2的单位）
    for _, listener in ipairs(Warrior2Listeners.list) do
...
-- 移除所有单位上的斗士buff
function modifier_warrior_2:RemoveAllWarriorBuffs()
    if not IsServer() then return end
    ...
    if caster and not caster:IsNull() then
        ...
        for _, ally in pairs(allies) do
            if ally and not ally:IsNull() then
                if ally:HasModifier("modifier_warrior_2_effect") then
                    ally:RemoveModifierByName("modifier_warrior_2_effect")
                end
            end
        end
    end
...
function modifier_warrior_2:OnDestroy()
    if IsServer() then
        -- 停止定时器，防止重新添加buff
        ...
        -- 从监听器列表中移除（如果还在列表中）
        for i, listener in ipairs(Warrior2Listeners.list) do
            if listener == self then
                table.remove(Warrior2Listeners.list, i)
                break
            end
        end
    end
...
-- 单位创建事件处理函数（静态方法）
function modifier_warrior_2:OnNPCSpawned(event)
    if not IsServer() then return end
    ...
    if not unit:IsBuilding() and (unit:IsHero() or unit:IsCreature() or unit:IsOther()) then
        ...
        GameRules:GetGameModeEntity():SetThink(function()
            ...
            if #Warrior2Listeners.list == 0 then return nil end
                ...
                for _, listener in ipairs(Warrior2Listeners.list) do
                    if listener and not listener:IsNull() then
                        ...
                        if ability and not ability:IsNull() then
                            -- 检查单位是否已经有 modifier
                            if not unit:HasModifier("modifier_warrior_2_effect") then
                                -- 添加 modifier
                                local success, result = pcall(function()
                                    return unit:AddNewModifier(listener:GetCaster(), ability, "modifier_warrior_2_effect", {})
                                end)
                                if success then
            ...
            return nil -- 停止定时器
        end, "Warrior2_UnitSpawn_" .. unit:GetEntityIndex(), 0.1)
    end
end

function modifier_warrior_2:OnIntervalThink()
    if IsServer() then
        -- 检查绑定单位是否还活着
        local caster = self:GetCaster()
...
function modifier_warrior_2:ApplyWarriorEffectToAllies()
    local caster = self:GetCaster()
    ...
    for _, ally in pairs(allies) do
        if ally and not ally:IsNull() then
            ...
            -- 检查是否已经有斗士生命值加成效果
            if not ally:HasModifier("modifier_warrior_2_effect") then
                -- 对于非英雄单位，确保单位已经完全初始化
                if not is_hero then
                    ...
                    if max_health <= 0 or max_health == nil then
                        ...
                        GameRules:GetGameModeEntity():SetThink(function()
                            if ally and not ally:IsNull() and not ally:HasModifier("modifier_warrior_2_effect") then
                                local success, result = pcall(function()
                                    return ally:AddNewModifier(caster, ability, "modifier_warrior_2_effect", {})
                                end)
                                if not success then
                                    -- 添加 modifier 失败，静默处理
                                end
                            ...
                            return nil
                        end, "Warrior2_DelayedAdd_" .. ally:GetEntityIndex(), 0.2)
                    else
                        -- 最大生命值有效，直接添加
                        local success, result = pcall(function()
                            return ally:AddNewModifier(caster, ability, "modifier_warrior_2_effect", {})
                    ...
                    if max_health <= 0 or max_health == nil then
                        ...
                        GameRules:GetGameModeEntity():SetThink(function()
                            ...
                            return nil
                        end, "Warrior2_DelayedAdd_Hero_" .. ally:GetEntityIndex(), 0.1)
                    else
                        -- 最大生命值有效，直接添加
                        local success, result = pcall(function()
                            return ally:AddNewModifier(caster, ability, "modifier_warrior_2_effect", {})
...
-- 己方单位生命值加成效果修饰符
if modifier_warrior_2_effect == nil then
    modifier_warrior_2_effect = class({})
end

function modifier_warrior_2_effect:IsHidden()
    return false
end

function modifier_warrior_2_effect:IsPurgable()
    return false
end

function modifier_warrior_2_effect:IsDebuff()
    return false
end

function modifier_warrior_2_effect:DeclareFunctions()
    return {
...
function modifier_warrior_2_effect:OnCreated(params)
    if IsServer() then
        ...
        -- 检查该单位是否已经被处理过（计数为1）
        -- 如果已经处理过，直接返回，不再处理
        if Warrior2ProcessedUnits[unit_index] then
            return
        end
        
        -- 立即标记该单位正在处理中（防止重复处理）
        Warrior2ProcessedUnits[unit_index] = true
        ...
        GameRules:GetGameModeEntity():SetThink(function()
            if not parent or parent:IsNull() then 
                -- 如果单位无效，清除标记
                Warrior2ProcessedUnits[unit_index] = nil
                return nil 
            end
            ...
            -- 从技能配置获取生命值加成
            local ability = self:GetAbility()
            local health_bonus = 250.0
            local warrior_health_bonus = 500.0
            
            if ability and not ability:IsNull() then
                health_bonus = ability:GetSpecialValueFor("health_bonus") or 250.0
                warrior_health_bonus = ability:GetSpecialValueFor("warrior_health_bonus") or 500.0
            end
            ...
            if not Warrior2OriginalHealth[unit_index] then
                -- 直接记录当前值作为原始值（假设这是第一次应用）
                Warrior2OriginalHealth[unit_index] = current_max_health
            end
            
            -- 获取原始最大生命值
            local original_max_health = Warrior2OriginalHealth[unit_index]
            ...
            GameRules:GetGameModeEntity():SetThink(function()
                ...
                return nil -- 停止定时器
            end, "Warrior2_ApplyHealth_" .. unit_index, 0.05)
            
            return nil -- 停止定时器
        end, "Warrior2_InitHealth_" .. unit_index, delay)
    end
end

-- 当 modifier 被添加时，立即检查并恢复生命值（防止被重置）
function modifier_warrior_2_effect:OnModifierAdded(params)
    if not IsServer() then return end
    ...
    if parent and not parent:IsNull() then
        GameRules:GetGameModeEntity():SetThink(function()
            ...
            return nil
        end, "Warrior2_OnModifierAdded_" .. parent:GetEntityIndex(), 0.1)
    end
end

-- 当 modifier 被移除时，立即检查并恢复生命值（防止被重置）
function modifier_warrior_2_effect:OnModifierRemoved(params)
    if not IsServer() then return end
    ...
    if parent and not parent:IsNull() then
        GameRules:GetGameModeEntity():SetThink(function()
            ...
            return nil
        end, "Warrior2_OnModifierRemoved_" .. parent:GetEntityIndex(), 0.1)
    end
end

-- 恢复生命值的辅助函数（供事件回调和OnIntervalThink使用）
function modifier_warrior_2_effect:RestoreHealthIfNeeded()
    if not IsServer() then return end
    ...
    if not Warrior2ProcessedUnits[unit_index] then
        return
    end
...
function modifier_warrior_2_effect:OnIntervalThink()
    if IsServer() then
        local parent = self:GetParent()
        if parent and not parent:IsNull() and parent:IsAlive() then
            local unit_index = parent:GetEntityIndex()
...
function modifier_warrior_2_effect:OnDestroy()
    if IsServer() then
        local parent = self:GetParent()
        if parent and not parent:IsNull() then
            local unit_index = parent:GetEntityIndex()
            ...
            if original_max_health then
                ...
                Warrior2ProcessedUnits[unit_index] = nil
                Warrior2OriginalHealth[unit_index] = nil
            end
        end
    end
end

function modifier_warrior_2_effect:GetTexture()
    return "item_heart"
end
