-- 战斗狂人1羁绊技能 - 全局buff技能
-- 场上有单位死亡时，战斗狂人单位获得5秒的10%攻击速度提升和20%的全能增伤（普攻和技能均增伤）
-- 如果在获得BUFF时战斗狂人单位的血量小于50%，则buff持续时间为10秒
-- 战斗狂人单位：战争之矛(mars1)、斧王(axe1)、敌法师(anti_mage1)、孽主(underlord1)

LinkLuaModifier("modifier_berserker_1", "heroes/universal/berserker_1", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_berserker_1_buff", "heroes/universal/berserker_1", LUA_MODIFIER_MOTION_NONE)
...
if berserker_1 == nil then
    berserker_1 = class({})
end

function berserker_1:GetIntrinsicModifierName()
    return "modifier_berserker_1"
end

function berserker_1:GetAbilityTextureName()
    return "axe_counter_helix_unleashed"
end

-- 全局战斗狂人羁绊效果修饰符
if modifier_berserker_1 == nil then
    modifier_berserker_1 = class({})
end

function modifier_berserker_1:IsHidden()
    return false
end

function modifier_berserker_1:IsPurgable()
    return false
end

function modifier_berserker_1:IsDebuff()
    return false
end

function modifier_berserker_1:IsPassive()
    return true
end

function modifier_berserker_1:GetTexture()
    return "axe_counter_helix_unleashed"
end

-- 战斗狂人单位列表
function modifier_berserker_1:IsBerserkerUnit(unit)
...
function modifier_berserker_1:OnCreated()
    if IsServer() then
        -- 只对玩家团队生效，敌人团队的羁绊不生效
        ...
        self.damage_amplification_percent = self:GetAbility():GetSpecialValueFor("damage_amplification_percent") or 20.0
...
function modifier_berserker_1:OnDestroy()
    if IsServer() then
        -- 从监听器列表中移除
...
function modifier_berserker_1:OnUnitDeath(dead_unit)
    if not IsServer() then return end
    ...
    for _, ally in pairs(allies) do
        if ally:IsAlive() and not ally:IsNull() then
            ...
            if self:IsBerserkerUnit(ally) then
                ...
                local buff_modifier = ally:FindModifierByName("modifier_berserker_1_buff")
                if not buff_modifier then
                    -- 如果还没有 modifier，添加永久 modifier
                    buff_modifier = ally:AddNewModifier(caster, self:GetAbility(), "modifier_berserker_1_buff", {})
                end
...
-- 战斗狂人单位buff修饰符
if modifier_berserker_1_buff == nil then
    modifier_berserker_1_buff = class({})
end

function modifier_berserker_1_buff:IsHidden()
    return false
end

function modifier_berserker_1_buff:IsPurgable()
    return false  -- 改为不可驱散，使用永久 modifier
end

function modifier_berserker_1_buff:IsDebuff()
    return false
end

function modifier_berserker_1_buff:DeclareFunctions()
    return {
...
function modifier_berserker_1_buff:OnCreated(params)
    -- 客户端和服务器端都需要初始化
    -- 从技能配置获取数值
    local ability = self:GetAbility()
    if ability and not ability:IsNull() then
        self.attack_speed_bonus_percent = ability:GetSpecialValueFor("attack_speed_bonus_percent") or 10.0
        self.damage_amplification_percent = ability:GetSpecialValueFor("damage_amplification_percent") or 20.0
    else
        self.attack_speed_bonus_percent = 10.0
        self.damage_amplification_percent = 20.0
    end
...
function modifier_berserker_1_buff:OnIntervalThink()
    if IsServer() then
        -- 检查 buff 是否应该失效
...
function modifier_berserker_1_buff:GetModifierAttackSpeedBonus_Constant()
...
-- 提供全能增伤（普攻和技能都增伤）
function modifier_berserker_1_buff:GetModifierTotalDamageOutgoing_Percentage()
    -- 只在 buff 激活时提供加成（stack = 1 表示激活）
    if self:GetStackCount() ~= 1 then
        return 0
    end
    
    return self.damage_amplification_percent or 20.0
end

function modifier_berserker_1_buff:GetTexture()
    return "item_bfury"
end
