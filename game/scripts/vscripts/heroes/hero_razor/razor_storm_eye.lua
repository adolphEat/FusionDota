...
razor_storm_eye = class({})

-- 自走棋式自动施法功能
function razor_storm_eye:OnUpgrade()
...
function razor_storm_eye:OnSpellStart()
    if not IsServer() then return end
    ...
    -- 启动Think函数来持续造成伤害
    local think_name = "StormEyeThink_" .. GameRules:GetGameTime() .. "_" .. math.random(10000)
    GameRules:GetGameModeEntity():SetThink(function()
        return StormEyeThink(storm_data)
    end, think_name, 0.1)
end

-- 全局Think函数：持续造成伤害
function StormEyeThink(data)
